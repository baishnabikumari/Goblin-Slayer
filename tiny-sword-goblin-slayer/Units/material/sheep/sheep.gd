...
# scenes
#========
@export var meat_scene=preload("res://Units/material/meat/meat.tscn")
@export var baby_sheep_scene=preload("res://Units/material/sheep/sheep.tscn")
...
func _ready() -> void:
	print($zone.is_connected("area_entered", Callable(self, "_on_zone_area_entered")))
	z_index = 4
	add_to_group("sheep")
	_apply_genes()
	print("initial life:", life)
	_assign_pack()
...
func _grow(delta):
	...
	var t = clamp(age / adult_age, 0.0, 1.0)
...
func take_damage(attack_pos: Vector2):
	print("take_damage called. life before:", life)
	life -= 1
	print("life after:", life)
	if hit_audio != null and not hit_audio.playing:
		hit_audio.play()
	_flash_red()
	if life <= 0:
		print("life <= 0 -> die()")
...
func _enter_graze():
	state = GRAZE
	anim.play("grass")
	if sheep_audio != null and not sheep_audio.playing:
		sheep_audio.play()
